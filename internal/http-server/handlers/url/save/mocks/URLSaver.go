...
// SaveURL provides a mock function with given fields: urToSave, alias
func (_m *URLSaver) SaveURL(urToSave string, alias string) (int64, error) {
	ret := _m.Called(urToSave, alias)
	...
	if rf, ok := ret.Get(0).(func(string, string) (int64, error)); ok {
		return rf(urToSave, alias)
	}
	if rf, ok := ret.Get(0).(func(string, string) int64); ok {
		r0 = rf(urToSave, alias)
	} else {
		r0 = ret.Get(0).(int64)
	}

	if rf, ok := ret.Get(1).(func(string, string) error); ok {
		r1 = rf(urToSave, alias)
	} else {
		r1 = ret.Error(1)
	}
